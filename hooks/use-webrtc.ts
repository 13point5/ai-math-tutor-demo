...
import { getSystemPromptV2 } from "@/lib/prompts";
...
export default function useWebRTCAudioSession(
  ...
  /**
   * Get the current game state as a string to include in messages
   */
  function getGameStateDescription(): string {
    const state = gameStateRef.current;
    return `
CURRENT GAME STATE:

Max:
- Number of pencils per group: ${state.max.numOfPencilsPerGroup}
- Number of groups: ${state.max.numOfGroups}

Lena:
- Number of pencils per group: ${state.lena.numOfPencilsPerGroup}
- Number of groups: ${state.lena.numOfGroups}
...
`;
    ...
    const systemPrompt = {
      ...
      item: {
        ...
        content: [
          {
            type: "input_text",
            text: getSystemPromptV2(question),
          },
        ],
      },
    };
    ...
    try {
      ...
      switch (msg.type) {
        ...
        case "conversation.item.input_audio_transcription.completed": {
          ...
          // Include game state information in the transcript
          const gameStateInfo = getGameStateDescription();
          const fullText = `Student says: ${transcript}\n--------\n${gameStateInfo}`;
          ...
          // Send a new message with the game state to the AI
        ...
        case "response.function_call_arguments.done": {
          console.log("FUNCTION CALL msg", msg);
  ...
  function sendTextMessage(text: string) {
    ...
    // Include game state in the text message
    const gameStateInfo = getGameStateDescription();
    const fullText = `${text}\n\n${gameStateInfo}`;
